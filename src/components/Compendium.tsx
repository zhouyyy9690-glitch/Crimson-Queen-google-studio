...
import { Crown, X, User, Network, List, Moon } from 'lucide-react';
...
export const Compendium = ({
  ...
  const foxChars = characters.filter(c => c.path === 'fox');
  const foxUnlocked = foxChars.filter(c => unlockedCharacters.has(c.id)).length;
  const foxProgress = foxUnlocked / (foxChars.length || 1);
          ...
          {viewMode === 'list' ? (
            <>
              ...
              <div className="w-full md:w-80 lg:w-96 border-r border-amber-900/10 flex flex-col bg-black/20">
                <div className="p-4 md:p-6 text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-neutral-600 font-display flex items-center justify-between border-b border-amber-900/5">
                  <div className="flex items-center gap-4">
                    <LunarPhaseIndicator progress={foxProgress} size={16} color="#444" />
                    <span>Soul Records / 魂灵志</span>
                  </div>
                ...
                <div className="flex-grow overflow-y-auto no-scrollbar px-4 pb-8 space-y-2">
                  {filteredCharacters.map((char) => {
                    ...
                    const scenes = [...history, currentSceneId];
                    const lastUpdate = char.updates?.slice().reverse().find(u => scenes.includes(u.atScene));
                    const displayName = lastUpdate?.name || char.name;
                    const displayNameEn = lastUpdate?.nameEn || char.nameEn;
                      ...
                      >
                        ...
                        <div className="relative shrink-0">
                           <div className={`w-10 h-10 md:w-12 md:h-12 border flex items-center justify-center transition-colors duration-700 ${
                             ...
                             {isUnlocked ? (
                               char.iconType === 'moon' ? (
                                 <LunarPhaseIndicator progress={foxProgress} size={28} color="#D4AF37" />
                               ) : (
                                 <EmbossedInitial nameEn={displayNameEn} className="text-xl md:text-2xl" />
                               )
                             ) : (
                    ...
                    >
                      ...
                      <div className="flex-grow min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="text-amber-600 font-display text-lg uppercase tracking-widest truncate">{name}</h4>
                          <span className="text-[8px] text-amber-900/60 uppercase tracking-[0.2em]">{nameEn}</span>
                        </div>
